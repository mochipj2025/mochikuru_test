(() => {
  ...
  if (world && petActor && petSprite && potActor && potSlime && heroMessage && timeToggle && nightStars) {
    ...
    /* ゲスト(hero-experience.js)の登場中は歩き回らず待機する */
    let guestHold = false;
    document.addEventListener('mochisura:guest', (event) => {
      guestHold = event.detail?.phase === 'enter';
      if (guestHold) petTarget = null;
    });

    /* 12段階の先読みは初期帯域を空けるため、手が空いてから */
    const preloadStages = () => stages.forEach(([file]) => {
      const image = new Image();
      image.src = `slime/images_pixel/${file}`;
    });
    if ('requestIdleCallback' in window) requestIdleCallback(preloadStages, { timeout: 4000 });
    else window.setTimeout(preloadStages, 2500);
    ...
    const animatePet = (time) => {
      ...
      } else if (time >= happyUntil && !guestHold) {
      ...
      if (time < happyUntil) {
        ...
        setPetFrame(0, Math.floor(time / 420) % 2);
      } else {
    ...
    /* ローダー(loading-experience.js)へ初期化完了を知らせる */
    world.dataset.motionReady = '1';
    document.dispatchEvent(new CustomEvent('mochisura:hero-ready'));
  }
})();
